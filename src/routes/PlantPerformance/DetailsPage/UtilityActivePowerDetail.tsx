...
import { ActivePowerDataPoint, TimePeriod, UtilityType } from "@/utils/types"
...
const UtilityActivePowerDetail: React.FC<Props> = ({ type }) => {
    ...
    const initLiveConnection = async () => {
      ...
      try {
        ...
        connection.on(config.liveEvent, (payload: any) => {
          ...
          const newPoint: ActivePowerDataPoint = {
          ...
          setActivePowerData((prev) => [...prev.slice(-6), newPoint])
    ...
    const fetchGraphData = async () => {
      try {
        const res = await api.get(
          `/AllGraphs/${config.detailedGraphByFilterId}?filter=${selectedTimePeriod}&${config.graphIdParam}=${id}&graphType=activePower`,
        )
        if (res.data) {
          ...
          if (res.data.graphData) {
            setActivePowerData(
              res.data.graphData.map((item: any) => ({
                ...
                kW1: Number(item.kW1 ?? 0),
                kW2: Number(item.kW2 ?? 0),
                kW3: Number(item.kW3 ?? 0),
              })),
            )
          } else setActivePowerData([])
        }
      } catch (err) {
  ...
  return (
    <div className="space-y-4">
      ...
      {serverDown && selectedTimePeriod === "live" ? (
        ...
        activePowerChartConfig.map((chartConfig) => (
          <div key={chartConfig.id} className="mb-2 cursor-pointer">
            ...
            <UtilityDetailsPanel
              chartConfigs={activePowerChartConfig}
...
export default UtilityActivePowerDetail
