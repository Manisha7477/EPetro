...
import { ApparentPowerDataPoint, TimePeriod, UtilityType } from "@/utils/types"
...
const UtilityApparentPowerDetail: React.FC<Props> = ({ type }) => {
    ...
    const initLiveConnection = async () => {
      ...
      try {
        ...
        connection.on(config.liveEvent, (payload: any) => {
          ...
          const newPoint: ApparentPowerDataPoint = {
          ...
          setApparentPowerData((prev) => [...prev.slice(-6), newPoint])
    ...
    const fetchGraphData = async () => {
      try {
        const res = await api.get(
          `/AllGraphs/${config.detailedGraphByFilterId}?filter=${selectedTimePeriod}&${config.graphIdParam}=${id}&graphType=apparentPower`,
        )
        if (res.data) {
          ...
          if (res.data.graphData) {
            setApparentPowerData(
              res.data.graphData.map((item: any) => ({
                ...
                kVA1: Number(item.kVA1 ?? 0),
                kVA2: Number(item.kVA2 ?? 0),
                kVA3: Number(item.kVA3 ?? 0),
              })),
            )
          } else setApparentPowerData([])
        }
      } catch (err) {
  ...
  return (
    <div className="space-y-4">
      ...
      {serverDown && selectedTimePeriod === "live" ? (
        ...
        apparentPowerChartConfig.map((chartConfig) => (
          <div key={chartConfig.id} className="mb-2 cursor-pointer">
            ...
            <UtilityDetailsPanel
              chartConfigs={apparentPowerChartConfig}
...
export default UtilityApparentPowerDetail
