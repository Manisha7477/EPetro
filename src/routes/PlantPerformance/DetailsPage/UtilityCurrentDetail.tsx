...
import { CurrentDataPoint, TimePeriod, UtilityType } from "@/utils/types"
...
const UtilityCurrentDetail: React.FC<Props> = ({ type }) => {
  ...
  const [currentData, setCurrentData] = useState<CurrentDataPoint[]>([])
    ...
    const initLiveConnection = async () => {
      ...
      try {
        ...
        connection.on(config.liveEvent, (payload: any) => {
          ...
          const newPoint: CurrentDataPoint = {
            ...
            i2: Number(payload.i ?? 0),
            i3: Number(payload.i3 ?? 0),
          }
          setCurrentData((prev) => [...prev.slice(-6), newPoint])
          lastLiveTsRef.current = Date.now()
          setServerDown(false)
        })

        await connection.invoke(config.joinGroupMethod, "current", Number(id))
    ...
    const fetchGraphData = async () => {
      try {
        const res = await api.get(
          `/AllGraphs/${config.detailedGraphByFilterId}?filter=${selectedTimePeriod}&${config.graphIdParam}=${id}&graphType=current`,
        )
        if (res.data) {
  ...
  return (
    <div className="space-y-4">
      ...
      {serverDown && selectedTimePeriod === "live" ? (
        ...
        currentChartConfig.map((chartConfig) => (
          <div key={chartConfig.id} className="mb-2 cursor-pointer">
            ...
            <UtilityDetailsPanel
              chartConfigs={currentChartConfig}
...
export default UtilityCurrentDetail
