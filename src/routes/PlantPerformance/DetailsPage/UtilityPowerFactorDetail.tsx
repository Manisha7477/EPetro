...
import { PowerFactorDataPoint, TimePeriod, UtilityType } from "@/utils/types"
...
const UtilityPowerFactorDetail: React.FC<Props> = ({ type }) => {
    ...
    const initLiveConnection = async () => {
      ...
      try {
        ...
        connection.on(config.liveEvent, (payload: any) => {
          ...
          const newPoint: PowerFactorDataPoint = {
          ...
          setPowerFactorData((prev) => [...prev.slice(-6), newPoint])
    ...
    const fetchGraphData = async () => {
      try {
        const res = await api.get(
          `/AllGraphs/${config.detailedGraphByFilterId}?filter=${selectedTimePeriod}&${config.graphIdParam}=${id}&graphType=powerFactor`,
        )
        if (res.data) {
          ...
          if (res.data.graphData) {
            setPowerFactorData(
              res.data.graphData.map((item: any) => ({
                ...
                pF1: Number(item.pF1 ?? 0),
                pF2: Number(item.pF2 ?? 0),
                pF3: Number(item.pF3 ?? 0),
              })),
            )
          } else setPowerFactorData([])
        }
      } catch (err) {
  ...
  return (
    <div className="space-y-4">
      ...
      {serverDown && selectedTimePeriod === "live" ? (
        ...
        powerFactorChartConfig.map((chartConfig) => (
          <div key={chartConfig.id} className="mb-2 cursor-pointer">
            ...
            <UtilityDetailsPanel
              chartConfigs={powerFactorChartConfig}
...
export default UtilityPowerFactorDetail
