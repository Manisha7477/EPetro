...
import { ReactivePowerDataPoint, TimePeriod, UtilityType } from "@/utils/types"
...
const UtilityReactivePowerDetail: React.FC<Props> = ({ type }) => {
    ...
    const initLiveConnection = async () => {
      ...
      try {
        ...
        connection.on(config.liveEvent, (payload: any) => {
          ...
          const newPoint: ReactivePowerDataPoint = {
          ...
          setReactivePowerData((prev) => [...prev.slice(-6), newPoint])
    ...
    const fetchGraphData = async () => {
      try {
        const res = await api.get(
          `/AllGraphs/${config.detailedGraphByFilterId}?filter=${selectedTimePeriod}&${config.graphIdParam}=${id}&graphType=reactivePower`,
        )
        if (res.data) {
          ...
          if (res.data.graphData) {
            setReactivePowerData(
              res.data.graphData.map((item: any) => ({
                ...
                kVAr1: Number(item.kVAr1 ?? 0),
                kVAr2: Number(item.kVAr2 ?? 0),
                kVAr3: Number(item.kVAr3 ?? 0),
              })),
            )
          } else setReactivePowerData([])
        }
      } catch (err) {
  ...
  return (
    <div className="space-y-4">
      ...
      {serverDown && selectedTimePeriod === "live" ? (
        ...
        reactivePowerChartConfig.map((chartConfig) => (
          <div key={chartConfig.id} className="mb-2 cursor-pointer">
            ...
            <UtilityDetailsPanel
              chartConfigs={reactivePowerChartConfig}
...
export default UtilityReactivePowerDetail
