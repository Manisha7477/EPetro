...
import { KvahEnergyDataPoint, TimePeriod, UtilityType } from "@/utils/types"
...
const UtilityEnergyConsumptionKvahDetail: React.FC<Props> = ({ type }) => {
  ...
  const [kvahData, setKvahData] = useState<KvahEnergyDataPoint[]>([])
    ...
    const initLiveConnection = async () => {
      ...
      try {
        ...
        connection.on(config.liveEvent, (payload: any) => {
          ...
          const newPoint: KvahEnergyDataPoint = {
          ...
          setKvahData((prev) => [...prev.slice(-6), newPoint])
          lastLiveTsRef.current = Date.now()
          setServerDown(false)
        })

        await connection.invoke(config.joinGroupMethod, "kvah", Number(id))
    ...
    const fetchGraphData = async () => {
      try {
        const res = await api.get(
          `/AllGraphs/${config.detailedGraphByFilterId}?filter=${selectedTimePeriod}&${config.graphIdParam}=${id}&graphType=kvah`,
        )
        if (res.data) {
          ...
          if (res.data.graphData) {
            setKvahData(
              res.data.graphData.map((item: any) => ({
                ...
                kVAh1: Number(item.kVAh1 ?? 0),
                kVAh2: Number(item.kVAh2 ?? 0),
                kVAh3: Number(item.kVAh3 ?? 0),
              })),
            )
          } else setKvahData([])
        }
  ...
  return (
    <div className="space-y-4">
      ...
      {serverDown && selectedTimePeriod === "live" ? (
        ...
        kvahEnergyChartConfig.map((chartConfig) => (
          <div key={chartConfig.id} className="mb-2 cursor-pointer">
            ...
            <UtilityDetailsPanel
              chartConfigs={kvahEnergyChartConfig}
...
export default UtilityEnergyConsumptionKvahDetail
