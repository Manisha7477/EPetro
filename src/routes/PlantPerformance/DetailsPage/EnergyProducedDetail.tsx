...
interface EnergyProducedDetailProps {
  utilityType: UtilityType // Only pass this when rendering
}

const EnergyProducedDetail: React.FC<EnergyProducedDetailProps> = ({
  utilityType,
}) => {
  const { plantName, id } = useParams<{ plantName: string; id: string }>()
  const navigate = useNavigate()

  //  get config for given utility
  const {
    apiEndpoint,
    hubUrl,
    pastDataMethod,
  ...
  // always "energyProduced"
  const chartId = "energyProduced"
  ...
  const fetchUtilityDetails = async (utilityId: string) => {
    try {
      ...
      if (res.data && res.data.length > 0) {
        ...
        //   // gensetId: String(data[`${utilityType.toLowerCase()}Id`]),
      ...
      if (res.data) {
        ...
        if (res.data.graphData) {
          const formatted = res.data.graphData.map((item: any) => {
            ...
            return {
              timestamp: label,
              total_net_kWh_Export: Number(item.total_net_kWh_Export ?? 0),
            }
          })
  ...
  // main effect: live/past switching
  useEffect(() => {
    if (!id) return
      ...
      try {
        ...
        try {
          ...
          if (!abortController.signal.aborted && Array.isArray(pastData)) {
            ...
            if (lastPoint)
              setLiveSummary({
                Avgtotal_net_kWh_Export: lastPoint.total_net_kWh_Export,
              })
          }
        } catch (err) {
        ...
        connection.on(liveEvent, (payload: any) => {
          ...
          setServerDown(false)
          setLiveSummary({
            Avgtotal_net_kWh_Export: newPoint.total_net_kWh_Export,
          })
        })
...
export default EnergyProducedDetail
