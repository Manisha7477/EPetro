...
import { FrequencyDataPoint, TimePeriod, UtilityType } from "@/utils/types"
...
const UtilityFrequencyDetail: React.FC<Props> = ({ type }) => {
    ...
    const initLiveConnection = async () => {
      ...
      try {
        ...
        connection.on(config.liveEvent, (payload: any) => {
          ...
          const newPoint: FrequencyDataPoint = {
          ...
          setFrequencyData((prev) => [...prev.slice(-6), newPoint])
          lastLiveTsRef.current = Date.now()
          setServerDown(false)
        })

        await connection.invoke(config.joinGroupMethod, "frequency", Number(id))
    ...
    const fetchGraphData = async () => {
      try {
        const res = await api.get(
          `/AllGraphs/${config.detailedGraphByFilterId}?filter=${selectedTimePeriod}&${config.graphIdParam}=${id}&graphType=frequency`,
        )
        if (res.data) {
          ...
          if (res.data.graphData) {
            setFrequencyData(
              res.data.graphData.map((item: any) => ({
                ...
                frequency1: Number(item.frequency1 ?? 0),
              })),
            )
  ...
  return (
    <div className="space-y-4">
      ...
      {serverDown && selectedTimePeriod === "live" ? (
        ...
        frequencyChartConfig.map((chartConfig) => (
          <div key={chartConfig.id} className="mb-2 cursor-pointer">
            ...
            <UtilityDetailsPanel
              chartConfigs={frequencyChartConfig}
...
export default UtilityFrequencyDetail
