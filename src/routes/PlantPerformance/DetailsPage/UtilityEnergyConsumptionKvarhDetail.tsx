...
import { KvarhEnergyDataPoint, TimePeriod, UtilityType } from "@/utils/types"
...
const UtilityEnergyConsumptionKvarhDetail: React.FC<Props> = ({ type }) => {
  ...
  const [kvarhData, setKvarhData] = useState<KvarhEnergyDataPoint[]>([])
    ...
    const initLiveConnection = async () => {
      ...
      try {
        ...
        connection.on(config.liveEvent, (payload: any) => {
          ...
          const newPoint: KvarhEnergyDataPoint = {
          ...
          setKvarhData((prev) => [...prev.slice(-6), newPoint])
          lastLiveTsRef.current = Date.now()
          setServerDown(false)
        })

        await connection.invoke(config.joinGroupMethod, "kvarh", Number(id))
    ...
    const fetchGraphData = async () => {
      try {
        const res = await api.get(
          `/AllGraphs/${config.detailedGraphByFilterId}?filter=${selectedTimePeriod}&${config.graphIdParam}=${id}&graphType=Kvarh`,
        )
        if (res.data) {
          ...
          if (res.data.graphData) {
            setKvarhData(
              res.data.graphData.map((item: any) => ({
                ...
                kvarh1: Number(item.kvarh1 ?? 0),
                kvarh2: Number(item.kvarh2 ?? 0),
                kvarh3: Number(item.kvarh3 ?? 0),
              })),
            )
          } else setKvarhData([])
        }
  ...
  return (
    <div className="space-y-4">
      ...
      {serverDown && selectedTimePeriod === "live" ? (
        ...
        kvarhEnergyChartConfig.map((chartConfig) => (
          <div key={chartConfig.id} className="mb-2 cursor-pointer">
            ...
            <UtilityDetailsPanel
              chartConfigs={kvarhEnergyChartConfig}
...
export default UtilityEnergyConsumptionKvarhDetail
