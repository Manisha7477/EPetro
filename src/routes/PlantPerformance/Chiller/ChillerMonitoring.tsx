...
const ChillerMonitoring = () => {
  const { id, plantName, lineName } = useParams()
  console.log("Chiller ID:", id)
  ...
  return (
    <MonitoringDashboard
      ...
      receiveEvent="ReceiveChillerData"
      joinGroupMethod="JoinChillerGroup"
      leaveGroupMethod="LeaveChillerGroup"
      getPastDataMethod="GetChillerPastData"
      groupId={id}
      navigationPath={(chartId) =>
        `/plantPerformanceUtilities/${plantName}/${lineName}/chiller/${id}/monitoring/${chartId}`
      }
      plantName={plantName}
      lineName={lineName}
    />
...
export default ChillerMonitoring
