...
const BoilerMonitoring = () => {
  const { id, plantName, lineName } = useParams()
  console.log("Boiler ID:", id)
  ...
  return (
    <MonitoringDashboard
      ...
      receiveEvent="ReceiveBoilerData"
      joinGroupMethod="JoinBoilerGroup"
      leaveGroupMethod="LeaveBoilerGroup"
      getPastDataMethod="GetBoilerPastData"
      groupId={id}
      navigationPath={(chartId) =>
        `/plantPerformanceUtilities/${plantName}/${lineName}/boiler/${id}/monitoring/${chartId}`
      }
      plantName={plantName}
      lineName={lineName}
    />
