...
const AirCompressorInfoAndKPI = () => {
  const { id } = useParams()
  const [airCompressorInfo, setAirCompressorInfo] = useState<any | null>(null)
  ...
  const fetchAirCompressorDetails = async (updateId: string) => {
    try {
      const res = await api.get(
        `/AirCompressor/GetAllAirCompDetails_Id?AirCompId=${updateId}`,
      )
      console.log("API Response:", res.data)
      if (res.data && res.data.length > 0) {
        const data = res.data[0]
        const status =
          data.airCompStatus?.toLowerCase() === "active" ? "active" : "inactive"
        // Build the infoCard-compatible structure
        const dynamicAirCompressorInfo = {
          title: data.airCompName || "AirCompressor Details",
          // subtitle: data.plantName,
          columns: 2,
          status,
          items: [
            {
              label: "AirCompressor IP",
              value: data.airCompIP,
            },
            ...
            { label: "Compressor Type", value: data.compType },
            { label: "Disp. or Capacity", value: data.dispOrCapacity },
            {
              label: "Max. Working Pressure Range",
              value: data.maxWorkingPresRange,
            },
            {
              label: "Min. Working Pressure Range",
              value: data.minWorkingPresRange,
            },
            { label: "Motor Power", value: data.motorPower },
            { label: "Theorized Voltage", value: data.theorizedVltg },
            { label: "Theorized Phase", value: data.theorizedPhase },
            { label: "Theorized Frequency", value: data.theorizedFreq },
            { label: "Tank Volume", value: data.tankVolume },
            { label: "Compressor RPM ", value: data.rpmComp },
            { label: "Oil Type", value: data.oilType },
            { label: "Oil Quantity", value: data.oilQty },
          ],
        }

        setAirCompressorInfo(dynamicAirCompressorInfo)
      }
    } catch (error) {
      console.error("Error fetching AirCompressor details:", error)
      // } finally {
      //   setLoading(false)
    }
  }
  ...
  const fetchKpiData = async (updateId: string) => {
    ...
    try {
      const res = await api.get(
        `/AllKpi/GetAirCompKpisById?AirCompId=${updateId}`,
      )
      if (res.data) {
        setKpiData(res.data)
      }
      ...
      <div className="flex flex-col md:w-2/3 gap-2 overflow-y-auto max-h-screen pr-2">
        ...
        >
          <KpiCard
            ...
            kpivalue={`${kpiData.powerConsumption ?? 0} kWh`}
          />
          <KpiCard
            title="Temperature"
            kpivalue={`${kpiData.temperature ?? 0} °C`}
          />
          <KpiCard title="Pressure" kpivalue={`${kpiData.pressure ?? 0} Pa`} />
          <KpiCard
            title="AirFlow Rate"
            kpivalue={`${kpiData.airFlowRate ?? 0} `}
          />
        </motion.div>
        <motion.div
          variants={fadeInUp}
        ...
        >
          <KpiCard
            ...
            kpivalue={`${kpiData.specificPower ?? 0} kWh`}
          />
          <KpiCard
            title="Steam Generation Rate"
            kpivalue={`${kpiData.steamGenerationRate ?? 0} `}
          />
          <KpiCard
            title="Specific Energy Consumption"
            kpivalue={`${kpiData.specificEnergyConsumption ?? 0} kWh`}
          />
        </motion.div>
        {/* Gauge Charts */}
        <motion.div
        ...
        >
          <GaugeChart
            ...
            data={kpiData.compressorEfficiency}
            unit="%"
          />
        </motion.div>
      </div>
...
export default AirCompressorInfoAndKPI
