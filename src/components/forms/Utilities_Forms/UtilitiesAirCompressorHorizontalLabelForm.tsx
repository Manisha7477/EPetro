...
interface IAirCompressorHorizontalLabelFormProps {
...
const UtilitiesAirCompressorHorizontalLabelForm: React.FunctionComponent<
  IAirCompressorHorizontalLabelFormProps
> = ({
  formVariables,
  initialDefaultValueData,
  ...
  const createNewAirCompressorBlockInitialValues = () => {
      ...
      >
        {(
          ...
          <Form autoComplete="on">
            ...
            <FieldArray name="AirCompressorItem">
              {({ push, remove }: FieldArrayRenderProps) => (
                <>
                  {values.AirCompressorItem?.map(
                    (airCompressorBlock: any, index: number) => {
                      const prefixedFormVariables: IFormVariable[] =
                        rmFormVariables.map((variable) => ({
                          ...variable,
                          name: `AirCompressorItem.${index}.${variable.name}`,
                        }))

                      // Modified: Show only the first 3 variables (Plant Name, Site,Line  name) when collapsed
                      const displayedFormVariables =
                        airCompressorBlock.isCollapsed
                          ...
                          <div className="flex justify-between items-center mb-4">
                            ...
                            <div className="flex space-x-2">
                              ...
                              <button
                                ...
                                onClick={() =>
                                  setFieldValue(
                                    `AirCompressorItem.${index}.isCollapsed`,
                                    !airCompressorBlock.isCollapsed,
                                  )
                                }
                  ...
                  <div className="w-full mt-2">
                    <button
                      type="button"
                      onClick={() => {
                        push(createNewAirCompressorBlockInitialValues())
                        setTimeout(() => {
                          validateForm()
                        }, 0)
                      }}
...
export default UtilitiesAirCompressorHorizontalLabelForm
