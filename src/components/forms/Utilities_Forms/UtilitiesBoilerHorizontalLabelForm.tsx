...
interface IBoilerHorizontalLabelFormProps {
...
const UtilitiesBoilerHorizontalLabelForm: React.FunctionComponent<
  IBoilerHorizontalLabelFormProps
> = ({
  formVariables,
  initialDefaultValueData,
  ...
  const createNewBoilerBlockInitialValues = () => {
      ...
      >
        {(
          ...
          <Form autoComplete="on">
            ...
            <FieldArray name="BoilerItem">
              {({ push, remove }: FieldArrayRenderProps) => (
                <>
                  {values.BoilerItem?.map((boilerBlock: any, index: number) => {
                    const prefixedFormVariables: IFormVariable[] =
                      rmFormVariables.map((variable) => ({
                        ...variable,
                        name: `BoilerItem.${index}.${variable.name}`,
                      }))

                    // Modified: Show only the first 3 variables (Plant Name, Site,Line  name) when collapsed
                    const displayedFormVariables = boilerBlock.isCollapsed
                        ...
                        <div className="flex justify-between items-center mb-4">
                          ...
                          <div className="flex space-x-2">
                            ...
                            <button
                              ...
                              onClick={() =>
                                setFieldValue(
                                  `BoilerItem.${index}.isCollapsed`,
                                  !boilerBlock.isCollapsed,
                                )
                              }
                            ...
                            >
                              {boilerBlock.isCollapsed ? "Expand" : "Collapse"}
                            </button>
                            {/* Delete Button */}
                            <button
                              type="button"
                  ...
                  <div className="w-full mt-2">
                    <button
                      type="button"
                      onClick={() => {
                        push(createNewBoilerBlockInitialValues())
                        setTimeout(() => {
                          validateForm()
                        }, 0)
                      }}
...
export default UtilitiesBoilerHorizontalLabelForm
